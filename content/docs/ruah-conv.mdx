---
title: Converter
description: How do I make this API agent-sized?
---

> **How do I make this API agent-sized?**

`curate` is the headline. A 400-endpoint spec is hostile to an agent; collapse it into 5–10 task-shaped tools and print the definition token footprint. Conversion is the supporting engine.

<p align="center">
  <img src="/demos/ruah-conv-demo.gif" alt="ruah conv curate petstore into task-shaped tools" width="100%" />
</p>
...
Collapse a large spec into ≤10 task-shaped tools. Collection + item CRUD on the same resource become `managePets` with an `action` enum. Every drop has a reason.

```bash
ruah conv curate ./spec.yaml
ruah conv curate ./spec.yaml --preset standard --out ./curated
ruah conv generate ./spec.yaml --curate --json
ruah conv generate ./spec.yaml --plan ./curated/curation.json --output ./generated
```

**Presets:** `minimal` (5 read-only families), `standard` (default, ≤10), `full` (no cap, still drops deprecated). `--interactive` walks accept/split/drop on a TTY. `--plan` replays a saved `curation.json` and reports drift.

`--deferred` (MCP targets only) exposes `search_tools` + `get_tool_schema` + `invoke_tool` instead of every operation. Use it when the surface is still large after curation, or when you skip curation on a 100+ endpoint spec.
