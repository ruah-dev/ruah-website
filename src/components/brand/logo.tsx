...
export function Logo({
  ...
  const content = (
    <>
      <svg
        ...
        width={s.icon}
        height={s.icon}
        className="shrink-0 transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-105"
        aria-hidden="true"
      >
        <path
      ...
      {showText && (
        <span className={cn("font-brand font-medium tracking-wide text-warm-100", s.text)}>
          ruah
        </span>
      )}
    </>
  ...
  if (!href) {
    return <span className={cn("flex items-center gap-2.5 group", className)}>{content}</span>;
  }
