...
/**
 * Phantom — the Ruah mascot.
 *
 * Eight expression states map to design-system semantic colors:
 *   idle      — teal   (default brand)
 *   thinking  — lavender (AI contemplation)
 *   tracking  — teal + cursor-following eyes (handled outside this component)
 *   agent     — lavender (agent-owned indication)
 *   success   — sage   (task complete / merged)
 *   loading   — amber  (in-progress)
 *   warning   — amber
 *   error     — coral
 *
 * `eyeOffsetX` / `eyeOffsetY` lets parents drive eye direction (cursor tracking).
 * `noFloat` disables the idle drift (for static placements inside cards).
 */
...
type EyeShape = "round" | "arc" | "line" | "squint" | "sparkle" | "cross";

const EXPRESSION_CONFIG: Record<
  PhantomExpression,
  { bodyClass: string; eyeShape: EyeShape; glowClass: string }
> = {
  idle: { bodyClass: "text-ruah-400", eyeShape: "round", glowClass: "bg-ruah-400/20" },
  thinking: {
    bodyClass: "text-lavender-400",
    eyeShape: "arc",
    glowClass: "bg-lavender-400/20",
  },
  tracking: {
    bodyClass: "text-ruah-400",
    eyeShape: "round",
    glowClass: "bg-ruah-400/20",
  },
  agent: {
    bodyClass: "text-lavender-400",
    eyeShape: "round",
  ...
  success: {
    bodyClass: "text-sage-400",
    eyeShape: "arc",
    glowClass: "bg-sage-400/20",
  },
  loading: {
    bodyClass: "text-amber-400",
    eyeShape: "squint",
  ...
  error: {
    bodyClass: "text-coral-400",
    eyeShape: "cross",
    glowClass: "bg-coral-400/20",
  },
};
...
const EYE_FILL_CLASS = "fill-warm-100";

/** Render a single eye at (cx, cy) in the SVG viewBox. */
function Eye({
  cx,
  cy,
  shape,
    ...
    case "round":
      return (
        <ellipse
          ...
          rx={24.4}
          ry={31.5}
          className={EYE_FILL_CLASS}
        />
      );
    case "arc":
      // Upper arc — looks like a happy/content eye.
      return (
        <path
          d={`M ${cx - 24} ${cy + 4} Q ${cx} ${cy - 26} ${cx + 24} ${cy + 4}`}
          stroke="currentColor"
    ...
    case "sparkle":
      return (
        <g
          ...
          transform={`translate(${offsetX * 0.3} ${offsetY * 0.3})`}
        >
          <ellipse cx={dx} cy={dy} rx={24.4} ry={31.5} />
          <circle cx={dx - 8} cy={dy - 12} r={6} className="fill-surface-0" />
        </g>
      );
    case "cross":
      return (
...
export function Phantom({
  ...
  const { bodyClass, eyeShape, glowClass } = EXPRESSION_CONFIG[expression];

  const shouldFloat = !noFloat && !reduced;
  ...
  return (
    ...
    >
      ...
      {!noGlow && (
        <div
          className={`absolute inset-0 -m-6 rounded-full blur-3xl ${glowClass}`}
          aria-hidden="true"
        />
      )}
      ...
      <motion.svg
        ...
        className="relative"
        animate={
          shouldFloat ? { y: [0, -6, 0], rotate: [0, 1.5, -1.5, 0] } : undefined
        }
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
        aria-hidden="true"
      >
        <path d={BODY_PATH} fill="currentColor" className={bodyClass} />
        <Eye cx={237} cy={355.2} shape={eyeShape} offsetX={eyeOffsetX} offsetY={eyeOffsetY} />
        <Eye cx={363} cy={355.2} shape={eyeShape} offsetX={eyeOffsetX} offsetY={eyeOffsetY} />
      </motion.svg>
    </div>
  );
}
